...
function addOutput(grid) {
    ...
    addCallback('load', function (c, elementId) {
        ...
        try {
            ...
            if (!obj || !obj.offsetY || !obj.time || !obj.closedMorph || !obj.closingMorph || !obj.openMorph || !obj.openingMorph || !obj.soundClosing || !obj.soundOpening) {
                c.send(obj.soundUnlocked);
                throw new Error();
            }
            context.get('offsetY').value(obj.offsetY);
            context.data.setDouble('offsetY', obj.offsetY);
...
function addSettings(grid) {
    ...
    var row = col.row(1);
    row.current.h(20);
    row.label('Offset Y:').labelAnchor(0, 0.5);
    row.trackpad(-3).id('offsetY');
    var row = col.row(1);
    row.current.h(20);
...
function addMorph(grid, label, morph, visible) {
    ...
    var morph = col.morph(mappet.createMorph(morph));
